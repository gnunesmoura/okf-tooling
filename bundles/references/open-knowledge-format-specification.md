...
description: Local copy of the OKF v0.1 draft specification used by the tooling module and documentation.
tags:
  - okf
  - specification
  - reference
...
- `type` — A short string identifying the kind of concept. Consumers
  ...
  tolerate unknown types gracefully.
...
specific kind of relationship is conveyed by the surrounding prose, not
by the link itself. Consumers that build a graph view typically treat
...
sources SHOULD be listed under a `# Citations` heading at the bottom of
...
OKF differs primarily in being **specified** — pinning down the small set
of rules needed for interoperability without dictating tooling.
...
- A **major** version bump may make breaking changes.
